  ...
  return (
    <View className={`flex-1 justify-center items-center p-6 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <View className={`p-6 rounded-2xl ${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-xl max-w-md w-full items-center`}>
        <View className={`w-20 h-20 rounded-full items-center justify-center mb-6 ${isDarkMode ? 'bg-gray-700' : 'bg-green-50'}`}>
          <Ionicons
        ...
        <TouchableOpacity
          ...
          className={`px-6 py-3 rounded-full ${isDarkMode ? 'bg-green-600' : 'bg-green-500'}`}
        >
